...
def main(selection="user", headless=False, short_exec=False):
    ...
    takes input of target position show configuration of fetch arm in pybullet
    """
    ...
    targetY = args.y
    targetZ = args.z
    target_pos = [targetX, targetY, targetZ]
    print("target position specified by user: ", target_pos)
    ...
    if programmatic_pos or headless:
        ...
        if joint_pos is not None and len(joint_pos) > 0:
            print("Solution found. Setting new arm configuration.")
            print("initial arm position: ", fetch.get_eef_position())
            set_joint_positions(robot_id, arm_joint_ids, joint_pos)
            
            print("final arm position: ", fetch.get_eef_position())
        else:
            print("EE position not reachable.")
